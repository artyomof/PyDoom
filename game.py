...
class App:
	...
	def do_shortcut(self, event):
		k = event.key
		m = event.mod
		if k == K_x and m&KMOD_LMETA: print("cmd+X")
		elif k == K_x and m&KMOD_LALT: print("alt+X")
		elif k == K_x and m&KMOD_LCTRL: print("ctrl+X")
		elif k == K_x and m&KMOD_LMETA + KMOD_LSHIFT: print("cmd+shift+X")
		elif k == K_x and m&KMOD_LMETA + KMOD_LALT: print("cmd+alt+X")
		elif k == K_x and m&KMOD_LMETA + KMOD_LALT + KMOD_LSHIFT: print("cmd+alt+shift+X")
	def run(self):
		while App.running:
...
class Player(pygame.sprite.Sprite):
	def __init__(self, filename, x, y, animation_folder = None):
		...
		if animation_folder != None:
			...
			self.curr_right = []
			self.curr_jump = []
			self.sub_folders = [name for name in os.listdir(animation_folder) if os.path.isdir(os.path.join(animation_folder, name))]
	...
	def update(self):
		...
		for i in App.scene.Collidable_Objects:
			if pygame.Rect.colliderect(App.TP.rect, i.rect) and App.TP.rect.bottom >= i.rect.top + 1:
				...
				self.vel.y = 0
				self.jumping = False
			elif pygame.Rect.colliderect(App.TP.rect, i.rect) and (App.TP.rect.left <= i.rect.right + 1) and (App.TP.rect.left > i.rect.left):
				self.pos.x = i.rect.right + 1
				self.jumping = False
			elif pygame.Rect.colliderect(App.TP.rect, i.rect) and (App.TP.rect.right >= i.rect.left + 1) and (App.TP.rect.right <= i.rect.right + 1):
